...
from utils.processing_blip import BlipProcessor
...
class Blip(torch.nn.Module):
    def __init__(self, model_path, device, args=None, score_qa=False) -> None:
        ...
        self.device = device
        self.model = BlipForConditionalGeneration.from_pretrained(model_path, torch_dtype=torch.float16).to(self.device)
    ...
    def score(self, images, prompts, **kwargs):
        ...
        inputs = {key: inputs[key].to(self.device) for key in inputs.keys()}
